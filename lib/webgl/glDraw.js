...
[
	...
	"	if (flag != 0)", 
	"	{", 
	"		gl_FragColor = vec4(blur_sample(emissive_map, dimensions, tex_coord, depth, 8.), 1.);", 
	"		return;", 
	"	}", 
	...
	"	gl_FragColor = vec4(emissive, sample_color.w);", 
	"}"
].join("\n");
var glVertexData = 
[
...
function glGenerateEmissive(image, emissiveTexture, reliefTexture)
{
	if (!gl) return null;
	if (image == null || image == undefined || 
		emissiveTexture == null || emissiveTexture == undefined || 
		reliefTexture == null || reliefTexture == undefined) return false;

	/*gl.bindFramebuffer(gl.FRAMEBUFFER, glEmissveFrameBuffer);
	gl.bindRenderbuffer(gl.RENDERBUFFER, glRenderBuffer);
	glEmissveFrameBuffer.width = image.width;
	glEmissveFrameBuffer.height = image.height;*/
	...
	if (status == gl.FRAMEBUFFER_MISSING_ATTACHMENT) console.log('FRAMEBUFFER_MISSING_ATTACHMENT');
	...
	glSetUVBox(0, 0, 1, 1);
	glSetTextures(null, null, reliefTexture, emissiveTexture);
	glSetFlag(1);
...
function glInit(canvas)
{
	...
	glEmissveFrameBuffer = gl.createFramebuffer();
