...
" float samples = 0.0;",
" vec2 filter = vec2(cos(gl_PointCoord.x * pi * 1.33), sin(gl_PointCoord.y * pi * 0.71));",
...
" return sum.rgb / samples;",
"}",

"vec3 light_surface(vec3 position, vec3 relief, vec3 surface_color, vec3 surface_normal, vec3 light, vec3 falloff, vec3 light_color)",
"{",
...
" vec3 ambient = vec3(1., 1., 1.) * ambient_intensity;",
//" vec3 intensity = ambient + (diffuse * atten);",

" return surface_color * ((diffuse * atten));",
"}",
...
" if (has_relief_map) depth = calc_depth(sample_relief.rgb);",
// " color = sample_color.xyz;",
// " normal = normalize((sample_normal.xyz * 2.) - 1.);",
// " depth = (sample_relief.r + sample_relief.g + sample_relief.b) / 3.0;//calc_depth(sample_relief.rgb);",
...
" vec3 sum = vec3(0., 0., 0.);",
" if (num_of_lights > 0) sum += light_surface(position_surface, position_relief, color, normal, light1_relief, light1_falloff, light1_color);",
" if (num_of_lights > 1) sum += light_surface(position_surface, position_relief, color, normal, light2_relief, light2_falloff, light2_color);",
" if (num_of_lights > 2) sum += light_surface(position_surface, position_relief, color, normal, light3_relief, light3_falloff, light3_color);",
...
{
//console.log(diffuse, normal, relief, emissive);
...
function glSetFilter(filter)
{
gl.uniform2fv(glShaderUniforms.filter, filter);
}
...
//glShaderUniforms.filter = gl.getUniformLocation(glShaderProgram, "filter");
...
if (glDebug) console.log(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS))
