...
[
	...
	"	if (flipped != 0) tex_coord = vec2(1.0 - tex_coord.x, tex_coord.y);", 
...
//	"	if (sample_color.w <= 0.01) discard;", 
	...
	"	if (flipped != 0) normal = vec3(1.0 - normal.r, normal.g, normal.b);", 
...
function glSetFlipped(flipped)
{
	if (!gl) return null;
	if (glUsingFallback) return null;
	if (flipped) gl.uniform1i(glShaderUniforms.flipped, 1);
	else gl.uniform1i(glShaderUniforms.flipped, 0);
}
...
function glInit(canvas)
{
	...
	glShaderUniforms.flipped = gl.getUniformLocation(glShaderProgram, "flipped");
