...
[
	...
	//"uniform vec2 filter[" + glFilterSize + "];", 
	...
	"	return (surface_color * intensity);", 
	"}", 
...
//	"	if (sample_color.w <= 0.01) discard;", 
	...
	"	if (has_relief_map) emissive = sample_emissive.xyz;", 
	"	if (has_emissive_map) depth = calc_depth(sample_relief.rgb);", 
...
function glDraw(width, height, x, y, overlayColor, diffuse, normal, relief, emissive, light1, light2, light3, intensity)
{
	if (!gl) return null;
	...
	glSetFlag(1);
	gl.blendFunc(gl.ONE, gl.ONE);//gl.SRC_ALPHA, gl.ONE);
	glBatch();
}
function glDrawCell(width, height, x, y, cellw, cellh, column, row, overlayColor, diffuse, normal, relief, emissive, light1, light2, light3, intensity)
{
	if (!gl) return null;
