...
.defines(function(){
    BaseRoom = ig.Class.extend({
            ...
            if (ig.input.pressed('leftMouse'))
            {
                ...
                if (GameCore.activeContextMenu != null)
                {
                    if (GameCore.activeContextMenu.isMouseOver('inspect'))
                    {
                        ...
                        Wiki.goto(GameCore.activeContextMenu.connectingEntity.description);
                    }
        ...
        draw: function()
        {
            ...
            {
                var door = this.doors[i];
                if (door != null && door.connectingRoom) door.draw();
            }
            for (var i = 0; i < this.enemies.length; i++)
            {
                var enemy = this.enemies[i];
        ...
        onPlayerEnter : function()
        {
            if (!this.discovered)
            {
                ...
                if (this.name != 'Tyche') this.addEnemy();
                console.log(this);
            }
        ...
        },  getAllDoors : function()
        {
            var availableDoors = [];
            for(var a =0;a<4;a++){
                if(this.doors[a] != null)
                {
                    availableDoors.push(this.indexToDoor(a))
                }
            }
            return availableDoors;
        },
getAvailableDoors : function()
        {
            var notConnected = [];
            for(var a=0; a<4; a++)
            {
                if(this.doors[a] != null)
                {
                    if(this.doors[a].connectingRoom == null && this.doors[a].connectingDoor == null)
                        notConnected.push(this.indexToDoor(a))
                }
            }
            return notConnected;
        },

indexToDoor : function(index)
        {
            var door = null;
            switch (index)
            {
                case 0: door = "topRight"; break;
                case 1: door = "bottomRight"; break;
                case 2: door = "bottomLeft"; break;
                case 3: door = "topLeft"; break;
            }
            return door;
        },
