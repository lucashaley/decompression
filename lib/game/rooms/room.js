        ...
        init: function() {
            ...
            for(var col = 0; col < 10; col++){
				for(var row = 0; row < 10; row++){
					...
					tilePositionX += (canvas.width/2);

					var tilePositionY = ((row + col) * (this.tileSize.height / 2)) + this.heightOffset;
            ...
            //create the possibleroom doors (currently positions set according to grid of 10x10)
            this.doors = [];
            this.doors.push(new EntityDoor(this.tiles[50].x, this.tiles[50].y));
            this.doors.push(new EntityDoor(this.tiles[4].x, this.tiles[4].y));
            this.doors.push(new EntityDoor(this.tiles[59].x, this.tiles[59].y));
            this.doors.push(new EntityDoor(this.tiles[95].x, this.tiles[95].y));
            //mark where door is usable : door 0 is top left and goes clockwise around
            this.doors[0].available = true;
            this.doors[1].available = false;
            this.doors[2].available = false;
            this.doors[3].available = true;
		},
