...
.defines(function () {
    ...
    var menuButton = ig.Class.extend({
        ...
        draw: function() {
            ...
            glDraw(this.image.width, this.image.height, this.x, this.y, this.texture);
        }
    });
    ...
    LogoScreen = ig.Game.extend({
        ...
        draw: function() {
            ...
            glDraw(this.background.width, this.background.height, cntrX, cntrY, this.textures[0]);
        }
    });
    ...
    StartScreen = ig.Game.extend({
        ...
        draw: function() {
            ...
            glDraw(this.background.width, this.background.height, cntrX, cntrY, this.textures[0]);
            glDraw(this.title.width, this.title.height, cntrX, 30, this.textures[1]);
        ...
        update: function () {
            ...
            if( this.isMoving) {
                ...
                if(this.nextRoom.roomX == canvas.width/2) {
                    ...
                    this.chooseRoom(this.nextRoom);
                    
                    this.nextRoom.offscreen = true;
                    console.log(this.shipLayout);
                }
            }
        },

        transitionRoom: function(newRoomStartCorner) {
            this.isMoving = true;

            if(newRoomStartCorner == "lowLeft" || newRoomStartCorner == "highLeft") {
                this.moveDirX = 8;
            }
            else {
                this.moveDirX = -8;
            }
            if(newRoomStartCorner == "highRight" || newRoomStartCorner == "highLeft") {
                this.moveDirY = 5;
            }
            else {
                this.moveDirY = -5;
