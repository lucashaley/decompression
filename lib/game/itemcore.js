...
.defines(function(){
	EntityItemCore = ig.Entity.extend(
	{
		...
		requestJunk : function()
		{
            ...
            if (chance <= 0.05)
            {
            	Chat.push(ScriptCore.ItemShortCircuitActivate);
            	GameCore.activePlayer.takeDamage(3, false);
            }
            else if (chance <= 0.075)
            {
            	Chat.push(ScriptCore.ItemLaserTripWireActivate);
            	for (var i = 0; i < GameCore.explored.length; i++)
            	{
            		var room = GameCore.explored[i];
            		if (room != null && !room.hasEnemies) room.addEnemy();
            	}
            }
            else if (chance <= 0.010)
            {
            	Chat.push(ScriptCore.ItemTheyAwakenActivate);
            	GameCore.currentRoom.addBoss();
            }
			else return new JunkItem();
		}, 
