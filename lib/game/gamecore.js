...
.defines(function(){
	EntityGameCore = ig.Entity.extend({
		...
		KeyCardRedInCore : false,
		KeyCardGreenInCore : false,
		KeyCardBlueInCore : false,		
		...
		init : function(playerCount) {
            ...
            //Initialize all players to Civilian class
            ...
            itemList.appendItem(41, 'DiscoveryImplant');
            itemList.appendItem(42, 'DefenseImplant');
            itemList.appendItem(43, 'CommandImplant');
            itemList.appendItem(44, 'ReflexImplant');
            itemList.appendItem(45, 'QuantumImplant');
		...
		update : function() {
			this.parent();	
			this.activePlayerTimeLeft = this.activePlayerTimeMax - timer.delta();
			if(this.players[activePlayer].actions <= 0 || this.activePlayerTimeLeft < 0) {
				this.onTurnEnd();
			}		
		},
		...
		onPhaseStart : function() {
			...
			this.cyclesTilPhase = this.cyclesInPhase;
		},
		onPhaseEnd : function() {
		...
		onCycleEnd : function() {
			...
			this.cyclesTilPhase -= 1;
			if(this.cyclesTilPhase <= 0) {
				this.onPhaseEnd();
			}
			onCycleStart();	
		},		
		onTurnStart : function() {
			this.players[activePlayer].onTurnStart();
			activePlayerTimeLeft = activePlayerTimeMax;
		},
		onTurnEnd : function() {
			
			this.players[activePlayer].onTurnEnd();
			activePlayer += 1;
