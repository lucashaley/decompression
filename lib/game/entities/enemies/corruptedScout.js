...
.defines(function(){
	EntityEnemySmallRoamer = ig.Entity.extend({
		animSheet: new ig.AnimationSheet('media/enemies/enemy.png', 96, 192),
		size: {x: 96, y:192},	
		flip : false,		
		...
		type : "Corrupted Scout",
		description : "",

		// portrait image, images for pawn facing directions
		portrait: new ig.AnimationSheet('media/enemies/enemyPortrait.png', 240, 240),
		...
		onPlayerMisses : function(attackingPlayer) {
			this.parent();
			//On player miss, corrupted scout attacks and then retreats
		}		
	});
