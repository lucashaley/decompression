...
.defines(function(){
	EntityPlayer = ig.Entity.extend({
		...
		{
			if (targetEnemy.indestructible) return;
		...
		ability : function()
		{
			switch (this.abilityName)
			{
				...
				for (var i = 0; i < this.inventory.length; i++)
				{
					...
					{
						junkFound++;
						if (junkFound == 1) item1 = item;
						else if (junkFound == 2) item2 = item;
						break;
					}
				}
		...
		takeDamage : function(dmgAmount, isNeurotoxin)
		{
			...
			Chat.push(this.name + ScriptCore.PlayerDamage1 + dmgAmount + ScriptCore.PlayerDamage2);
